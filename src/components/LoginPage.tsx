...
import { LogIn, Phone, User, Landmark, Sparkles, Shield, UserCheck } from 'lucide-react';
...
export default function LoginPage({ members, onLogin, onRegisterAndLogin }: LoginPageProps) {
  ...
  // Extract FJKM/FPFi demo members from data for quick click-to-login helper
  const demoMembers = members.filter(m => m.churchId === 'fjkm-isotry').slice(0, 4);
  ...
  const handleQuickLogin = (m: Member) => {
    setName(m.name);
    setPhone(m.phone);
    setError('');
    // Auto-login with 100ms delay for feedback
    setTimeout(() => {
      onLogin(m);
    }, 150);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-slate-950 text-white font-sans overflow-y-auto p-6 relative">
      ...
      <div className="w-full text-center space-y-4 pt-8">
        <div className="mx-auto w-16 h-16 rounded-3xl bg-gradient-to-tr from-violet-600 to-indigo-700 flex items-center justify-center shadow-lg border border-violet-500/30">
        ...
        <div>
          ...
          <p className="text-[11px] text-slate-400 uppercase font-black tracking-widest mt-1">
            Protestante Fifohazana Isotry
          </p>
        </div>
      </div>

      {/* Main Login Form card */}
      <div className="w-full max-w-sm mx-auto bg-slate-900/80 backdrop-blur-md border border-slate-800 p-6 rounded-3xl shadow-xl space-y-5 my-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            className="w-full py-3 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white font-black text-xs uppercase tracking-wide rounded-xl shadow-md border-b-[3px] border-indigo-805 cursor-pointer active:scale-[0.98] transition-all flex items-center justify-center gap-1.5"
          >
      ...
      <div className="w-full max-w-sm mx-auto space-y-2 pb-6">
        <span className="block text-[9px] font-black text-slate-500 uppercase tracking-widest text-center">
          📱 TSIKIDY FIDIRANA HAINGANA (DEMO TEST ACCOUNTS):
        </span>
        <div className="grid grid-cols-2 gap-1.5 pl-1">
          {demoMembers.map((m) => {
            const isJean = m.name.toLowerCase().includes('jean');
            return (
              <button
                key={m.id}
                type="button"
                onClick={() => handleQuickLogin(m)}
                className={`p-2 rounded-xl text-left text-[11px] border cursor-pointer active:scale-[0.96] transition-all block ${
                  isJean
              ...
              >
                ...
                <div className="text-[9px] font-mono opacity-80 text-slate-400 pt-0.5">
                  {m.phone}
                </div>
              </button>
            );
